...
This evidence, based on a model and data from a previous scientific study, suggests at face value that researchers confronted with similar models should use PAL rather than previous particle filter methods.
We show that the evidence is flawed because the PAL calculations were carried out using a dataset scaled differently from the previous study.
...
bake(file = paste0(out_dir, "mif2_maximization/round_01/ovov_mif_01.rds"),{ 
  registerDoFuture()
...
bake(file = paste0(out_dir, "mif2_maximization/round_01/ovov_el_01.rds"),{
...
el_01 <- readRDS(paste0(out_dir,"mif2_maximization/round_01/ovov_el_01.rds"))
starting_values_02 <- choose_n_fits(el_01, top_n_fits, ncores)


bake(file = paste0(out_dir,"mif2_maximization/round_02/ovov_mif_02.rds"),{ 
  registerDoFuture()
  plan(multicore, workers=ncores)
...
bake(file = paste0(out_dir, "mif2_maximization/round_02/ovov_el_02.rds"),{
...
el_02 <- readRDS(paste0(out_dir, "mif2_maximization/round_02/ovov_el_02.rds"))
starting_values_03 <- choose_n_fits(el_02, top_n_fits, ncores)


bake(file = paste0(out_dir, "mif2_maximization/round_03/ovov_mif_03.rds"),{ 
  registerDoFuture()
  plan(multicore, workers=ncores)
...
bake(file = paste0(out_dir,"mif2_maximization/round_03/ovov_el_03.rds"),{
...
mif_el <- readRDS(paste0(out_dir,"mif2_maximization/round_03/ovov_el_03.rds")) 
...
<<pf-sim-100, echo=F, warning=F, message=F>>=
# Set WWR's MLE
wwr_mle_raw <- c(params_fixed, 
             "beta1" = 11.48,
             "beta2" = 0.25,
...
ovov_sim_100 <- readRDS(paste0(out_dir, "pfilter/simulated_dataset/ovov_sim_100.rds"))
bake(file=paste0(out_dir, "pfilter/simulated_dataset/pfilter_ovov_sim_100.rds"),{
  ...
  data.frame(time=c(1:416), ovov_sim_100[[i]]) |>  pomp(t0=0,
           time="time",
           rprocess=discrete_time(rproc,delta.t=1/4),
           rinit=rinit_wwr,
           dmeasure=dmeas,
    ...
    foreach(j=1:ncores, .combine=c,
      ...
      pfilter(sir_wwr[[i]],Np=NP_MIF,save.states=T, params=wwr_mle_raw)
    } -> pfs
...
bake(file=paste0(out_dir, "pfilter/simulated_dataset/palfilter_ovov_sim_100.rds"),{
  palfilter_ovov_sim_100 <- c()
  registerDoFuture()
  plan(multicore, workers=ncores)
  for(i in 1:length(ovov_sim_100)){
    pal_sim_dat <- ovov_sim_100[[i]] |> as.matrix()
    lik_list <- rotavirus_SMC_qropxi(init_dist,pal_sim_dat , 9, 
                                     ...
                                     t(prop), NP_MIF,1)
    palfilter_ovov_sim_100[i] <- lik_list$log_lik
  }
...
<<plot1,echo=FALSE, fig.height=4, fig.width=5.5, fig.cap='Log-likelihoods computed using two filtering methods for 100 randomly simulated datasets at the MLE of the OvOv model.The particle filter and PAL used 50,000 particles.The red line is the line $Lik(pfilter)=Lik(palfilter)$.On average, the particle filter likelihood is $7.7$ log units higher than the PAL. {This may not be wrong, since the likelihood evaluated by pfilter is not fully optimized by employing many particles.}'>>=
data <- data.frame(pfilter = pfilter_ovov_sim_100, palfilter = palfilter_ovov_sim_100)

ggplot(data, aes(x = pfilter, y = palfilter)) +
  geom_point(shape = 20, size = 0.3) +  # pch=20, cex=0.3 equivalent
  geom_abline(intercept = 0, slope = 1, color = "red") +
  labs(title = "PAL vs SMC: OvOv with 100 simulations",
       x = "pfilter",
       y = "palfilter") +
  theme_minimal()
@
...
<<pfilter_org, echo=FALSE, message=FALSE, warning=FALSE>>=
dat[-1, ] |>
  pomp(t0=0,
       time="time",
       rprocess=discrete_time(rproc,delta.t=1/4), 
...
bake(file=paste0(out_dir,"pfilter/original_dataset/pfilter_org.rds"),{
  registerDoFuture()
  plan(multicore, workers = ncores)
  foreach(i=1:ncores, .combine=c,
          .options.future=list(seed=998468235L)
  ) %dofuture% {
    pfilter(m1,Np=NP_MIF,params=wwr_mle_raw)
  } -> pfs
  pfs
})

bake(file=paste0(out_dir,"pfilter/original_dataset/pfilter_list_cond_lik.rds"),{
  list_cond_lik <- list()
  registerDoFuture()
  plan(multicore, workers = ncores)
  for(k in 1:ncores){
    list_cond_lik[[k]] <- pfs[[k]]@cond.logLik
  }
  list_cond_lik
})
@

<<palfilter_org, echo=FALSE>>=
...
Sys.time()
bake(file=paste0(out_dir,"pfilter/original_dataset/palfilter_org.rds"),{
  realdat <- ovov_sim_100[[i]] |> as.matrix()
  pal_lik_list_org <- rotavirus_SMC_qropxi(init_dist,realdat , 9, 
                                   c(initial_guess[1],initial_guess[2],
...
<<plot2, echo=FALSE, fig.height=4, fig.width=5.5, fig.cap='Conditional log-likelihoods computed using two methods for the real rotavirus dataset, with the SMC method replicated 36 times due to its high variances. The blue line represents overlapping results from 36 SMC computations, while the red line is derived from PAL. The main sources of likelihood shortfall of SMC are at time points $t=1, 2, 3, 11, 81, 194$, and $325-333$.'>>=
# Create your data
time <- 1:416
ncores <- length(list_cond_lik)

# Combine the data into a single dataframe for SMC
data_list <- lapply(1:ncores, function(i) {
  data.frame(time = time, cond_loglik = list_cond_lik[[i]], method = "SMC")
})
...
df_pal <- data.frame(time = time, cond_loglik = pal_lik_list_org$ll_storage, method = "PAL")

# Combine both data frames
df <- bind_rows(df_smc, df_pal)

# Plot with ggplot
ggplot(df, aes(x = time, y = cond_loglik, group = interaction(method, factor(1:ncores)), color = method)) +
  geom_line() +
  theme_minimal() +
  labs(title = "Conditional Log Likelihood Over Time",
       x = "Time",
       y = "Conditional Log Likelihood",
       color = "Method") +
  scale_color_manual(values = c("SMC" = "blue", "PAL" = "red")) +
...
\begin{tabular}{|c c c|} 
 \hline
  & PAL & SMC  \\
  \hline
 OvOv & \Sexpr{pal_lik_list_org$log_lik} & $\Sexpr{logmeanexp(sapply(pfs, logLik))}$ \\ 
 \hline
\end{tabular}
\end{table}
