...
NMIF         = switch(RUN_LEVEL, 5,    100)
ncores       = switch(RUN_LEVEL, 4,    36)
top_n_fits   = switch(RUN_LEVEL, 1,    12)
sim_rep      = switch(RUN_LEVEL, 5,    100) # Total number of simulations
...
el_01 <- readRDS(paste0(out_dir,"mif2_maximization/round_01/el_01.rds"))
...
el_02 <- readRDS(paste0(out_dir, "mif2_maximization/round_02/el_02.rds"))
...
el_03 <- readRDS(paste0(out_dir,"mif2_maximization/round_03/el_03.rds")) 
...
bake(file=paste0(out_dir, "pfilter/simulated_dataset/ovov_sim_100.rds"),{
  ...
  repeat {
    ...
    if (!any(sim_data == 0)) {
      ovov_sim_100[[length(ovov_sim_100) + 1]] <- sim_data
      i <- i + 1  
    } else {
      print(paste("Condition not met for iteration", i, "- retrying"))
    }
    
    if (length(ovov_sim_100) >= sim_rep) break  
  }
  ovov_sim_100
})
@
...
bake(file=paste0(out_dir, "pfilter/simulated_dataset/pfilter_ovov_sim_100.rds"),{
  ...
  for(i in 1:length(ovov_sim_100)){
    ...
    foreach(j=1:ncores, .combine=c,
      ...
      pfilter(pfilter_obj[[i]],Np=NP_MIF,save.states=T, params=params_wwr)
    } -> pfs
...
bake(file=paste0(out_dir,"pfilter/original_dataset/pfilter_wwr_cond_lik.rds"),{
  pfilter_wwr_cond_lik <- list()
  registerDoFuture()
  plan(multicore, workers = ncores)
  for(k in 1:ncores){
    pfilter_wwr_cond_lik[[k]] <- pfilter_wwr[[k]]@cond.logLik
  }
...
bake(file=paste0(out_dir,"pfilter/original_dataset/palfilter_lik_list_wwr.rds"),{
  palfilter_lik_list_wwr <- rotavirus_SMC_qropxi(init_dist,realdat , 9, 
    c(initial_guess[1],initial_guess[2],initial_guess[3],initial_guess[4],initial_guess[5]),     gamma_par = c(initial_guess[7],  1/initial_guess[7]), 
    norm_par = c(0.07,initial_guess[6]), 
    t(prop), NP_MIF,1)
...
data_list <- lapply(1:ncores, function(i) {
  data.frame(time = time, cond_loglik = pfilter_wwr_cond_lik[[i]], method = "SMC")
})
...
df_pal <- data.frame(time = time, cond_loglik = palfilter_lik_list_wwr$ll_storage, method = "PAL")
...
\begin{tabular}{|c c c|} 
 \hline
  & PAL & SMC  \\
  \hline
 OvOv & \Sexpr{palfilter_lik_list_wwr$log_lik} & $\Sexpr{logmeanexp(logLik(pfilter_wwr))}$ \\ 
 \hline
\end{tabular}
\end{table}
