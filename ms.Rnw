...
We discovered that WWR (unlike SBH) used a poorly fitting choice of initial conditions for the latent process in January 2001 (their time 0). 
...
THE TABLE SEEMS TO BE TALL AND THIN. SHOULD WE PUT IT IN A APPENDIX?}
