...
One can make allowance for the number of estimated parameters using a quantity such as Akaike's information criterion [REF].
...
On $\approx 1200$ data points ($3*416$), this corresponds to a discrepancy of around $-1200\log(0.07)=3200$ log-likelihood units, largely explaining the difference reported in Table~1 and interpreted by \cite{wwr} as evidence supporting PAL.
...
ggplot(
  df, 
  aes(
    x = time, y = cond_loglik, 
    group = interaction(method, factor(1:ncores)), 
    color = method
  )
) +
  geom_line() +
