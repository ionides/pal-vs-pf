...
\begin{table}[htbp]
  ...
  \begin{threeparttable}
    \caption{Maximum likelihood estimation of parameters by PAL \citep{wwr}, and by mif2 algorithm \citep{pomppackagepaper} for the OvOv model.}
    }
    \label{tab:mlebywwr}
    \begin{tabular}
      \toprule
      Parameter      & OvOv  & OvOv (\textbf{pomp})$^*$ \\
      \midrule
