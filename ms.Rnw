...
bake(file=paste0(out_dir, "pfilter/simulated_dataset/palfilter_ovov_100.rds"),{
  pal_log_lik <- c()
  registerDoFuture()
  plan(multicore, workers=ncores)
  for(i in 1:length(ovov_sim_100)){
    ...
    lik_list <- rotavirus_SMC_qropxi(init_dist,pal_sim_dat , 9, 
                                     ...
                                     t(prop), NP_MIF,1)
    pal_log_lik[i] <- lik_list$log_lik
  }
  pal_log_lik
})
...
\includegraphics[width=4.5in]{plot1}
