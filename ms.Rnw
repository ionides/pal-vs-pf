...
\centering 
\caption{AIC for the OvOv rotavirus model, computed using two filtering methods. PAL is the Poisson approximate likelihood, implemented using the code of WWR. PF is the particle filter, implemented using the R package pomp \citep{pomppackagepaper}. Lines 1, 2 and 7 are from WWR, and the remainder are our own computations. We used $5\times 10^4$ particle for both PF and PAL. PF was repeated 36 times to reduce the Monte Carlo variance, but this step was not necessary for PAL due to its lower Monte~Carlo variance. PF was maximized using iterated filtering, following the approach of SBH. PAL was maximized using coordinate gradient descent, using the code of WWR.
} 
...
\begin{tabular}{lllr} 
  ...
  \\
  5. & PF & Initial values changed to match SBH &
...
Thus, line 7 can properly be compared only to line 1 but not to any other line of  Table~\ref{tab:ovovrealdata}. 
...
Also contained additional results reinforcing the conclusions presented in this article.
...
<<maximization7-Stocks_rinit, include=FALSE>>=
set.seed(12345)
sd_07 = (2/3)^6
