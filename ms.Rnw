...
This evidence, based on a model and data from a previous scientific study by \citet{stocks}, might suggests that researchers confronted with similar models should use PAL rather than particle filter methods.
...
\begin{tabular}{lllllr} 
  ...
  \hline
  1. & PF  & SBH  & SBH   & Table~1 of WWR, originally from SBH &  \Sexpr{myround(20134.38,tab_signif)}
  \\
  2. & PAL & WWR  & WWR   & Table~1 of WWR                      & \Sexpr{myround(13778.08,tab_signif)}
  \\
  3. & PAL & WWR  & WWR   & Re-evaluating the data, parameters from WWR &
    \Sexpr{myround(-2*palfilter_lik_list_wwr$log_lik + 2*7 ,tab_signif)} 
  \\
  4. & PF  & WWR & WWR    & Parameters matching 3. &
    \Sexpr{myround(-2*logmeanexp(logLik(pfilter_wwr)) + 2*7,tab_signif)}
  \\
  5. & PF  & WWR & WWR modified   & Initial values changed to match SBH &
     \Sexpr{myround(-2*est_mle_vec_sbh["logLik"] + 2*7,tab_signif)}
  \\
  6. & PAL & WWR & WWR modified   & Data, model and parameters matching 5.  &
     \Sexpr{myround(-2*palfilter_mif2_mle$log_lik + 2*7 ,tab_signif)}
  \\   
  7. & ARMA & SBH & ARMA benchmark    & Sec.~7.3.2 of WWR & 23043
  \\
  8. & ARMA & WWR & ARMA benchmark      & ARMA benchmark &
...
\begin{tabular}{lllllr} 
  \hline
  & Method   & Data  & Model & Parameters \& Source                    & AIC  \\  
  \hline
  1. & PF      & SBH   & SBH   & Table~1 of WWR, originally from SBH &  \Sexpr{myround(20134.38,tab_signif)} \\
  2. & PAL     & WWR   & WWR   & Table~1 of WWR                      & \Sexpr{myround(13778.08,tab_signif)} \\
  3. & PAL     & WWR   & WWR   & Re-evaluating the data, parameters from WWR & \Sexpr{myround(-2*palfilter_lik_list_wwr$log_lik + 2*7 ,tab_signif)} \\
  4. & PF      & WWR   & WWR   & Parameters matching 3. & \Sexpr{myround(-2*logmeanexp(logLik(pfilter_wwr)) + 2*7,tab_signif)} \\
  5. & PF      & WWR   & WWR modified   & Initial values changed to match SBH & \Sexpr{myround(-2*est_mle_vec_sbh["logLik"] + 2*7,tab_signif)} \\
  6. & PAL     & WWR   & WWR modified   & Data, model and parameters matching 5. & \Sexpr{myround(-2*palfilter_mif2_mle$log_lik + 2*7 ,tab_signif)} \\
  \hline
     & Benchmark &  &  &  \\
  \hline
  7. & ARMA    & SBH   & \multicolumn{2}{c}{\centering Sec.~7.3.2 of WWR}             & 23043 \\
  8. & ARMA    & WWR   & \multicolumn{2}{c}{\centering Re-evaluating 7. with data used by WWR}                 & \Sexpr{myround(wwr_benchmark,tab_signif)} \\
  \hline
\end{tabular}
\end{table}
...
Comparison of lines 3 and 5 shows that this improvement enables PF to reach the values attained by PAL, and show a small improvement.
