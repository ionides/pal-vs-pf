...
This evidence, based on a model and data from a previous scientific study, might suggests that researchers confronted with similar models should use PAL rather than previous particle filter methods.
Alternatively, the improvement in likelihood demonstrated over the results presented by \cite{stocks} might indicate an error in that previous work.
...
If a poorly fitting specification of initial value parameters is amended, the particle filter also outperforms PAL on the actual data. Nevertheless, PAL still yields reliable results and demonstrates superiority in computational time.
\end{abstract}
