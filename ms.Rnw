...
<<OvOv-model, echo=False, massage=False, warning=False, results=hide>>=
# measurement model 
dmeas <- Csnippet("
                  if (ISNA(cases1)) {
                  lik = (give_log) ? 0 : 1;
...
<<maximization1, echo=False, massage=False, warning=False, results=hide>>=
set.seed(123)
sir_box <- rbind(
  beta1=c(10,15),
  beta2=c(0.2,0.4),
...
bake(file = paste0("output/round_01/RUN_LEVEL_",RUN_LEVEL,"/ovov_mif_01.rds"),{ 
  registerDoFuture()
...
bake(file = paste0("output/round_01/RUN_LEVEL_",RUN_LEVEL,"/ovov_el_01.rds"),{
...
<<maximization2, echo=False, massage=False, warning=False, results=hide>>=
set.seed(1234)
sd_02           = 2/3

el_01 <- readRDS(paste0("output/round_01/RUN_LEVEL_",RUN_LEVEL,"/ovov_el_01.rds"))
starting_values_02 <- choose_n_fits(el_01, top_n_fits, ncores)


bake(file = paste0("output/round_02/RUN_LEVEL_",RUN_LEVEL,"/ovov_mif_02.rds"),{ 
  registerDoFuture()
  plan(multicore, workers=ncores)
...
bake(file = paste0("output/round_02/RUN_LEVEL_",RUN_LEVEL,"/ovov_el_02.rds"),{
...
<<maximization3, echo=False, massage=False, warning=False, results=hide>>=
set.seed(12345)
sd_03           = (2/3)^2

el_02 <- readRDS(paste0("output/round_02/RUN_LEVEL_",RUN_LEVEL,"/ovov_el_02.rds"))
starting_values_03 <- choose_n_fits(el_02, top_n_fits, ncores)


bake(file = paste0("output/round_03/RUN_LEVEL_",RUN_LEVEL,"/ovov_mif_03.rds"),{ 
  registerDoFuture()
  plan(multicore, workers=ncores)
...
bake(file = paste0("output/round_03/RUN_LEVEL_",RUN_LEVEL,"/ovov_el_03.rds"),{
...
<<estimated-mle, echo=False, massage=False, warning=False>>=
mif_el <- readRDS(paste0("output/round_03/RUN_LEVEL_",RUN_LEVEL,"/ovov_el_03.rds")) 
...
<<pf-sim-100, echo=False, results=hide, massage=False, warning=False>>=
# Set WWR's MLE
...
bake(file=paste0("output/pfilter/ovov_sim_100_",RUN_LEVEL,".rds"),{
  ovov_sim_100 <- list()
  registerDoFuture()
  plan(multicore, workers=ncores)
...
<<pfilter_sim_100, echo=False, massage=False, warning=False, results=hide>>=
ovov_sim_100 <- readRDS(paste0("output/pfilter/ovov_sim_100_",RUN_LEVEL,".rds"))
bake(file=paste0("output/pfilter/pfilter_ovov_sim_100_",RUN_LEVEL,".rds"),{
...
<<palfilter_sim_100, echo=False, massage=False, warning=False, results=hide>>=
readRDS(paste0("output/pfilter/ovov_sim_100_",RUN_LEVEL,".rds"))
prop <- 420
...
bake(file=paste0("output/pfilter/palfilter_ovov_100_",RUN_LEVEL,".Rdata"),{
  y <- c()
  registerDoFuture()
  plan(multicore, workers=ncores)
  for(i in 1:length(ovov_sim_100)){
...
\begin{table}[htbp]
  ...
  \begin{threeparttable}
    ...
    \begin{tabular}{>{\raggedright\arraybackslash}p{2cm} *{3}{>{\centering\arraybackslash}p{1.5cm}} >{\centering\arraybackslash}p{2.5cm}}
      ...
      \(\sigma^2_q\) & n/a   & 0.042 & 0.021 & $\Sexpr{round(est_mle_vec["sigma_q"],2)}$\\
