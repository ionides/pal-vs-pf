...
RUN_LEVEL = 1
NP_MIF       = switch(RUN_LEVEL, 5000, 50000)
...
<<OvOv-model, include=False, massage=False, warning=False>>=
# measurement model 
dmeas <- Csnippet("
                  if (ISNA(cases1)) {
                  lik = (give_log) ? 0 : 1;
...
<<maximization1, include=False, massage=False, warning=False>>=
set.seed(123)
sir_box <- rbind(
  beta1=c(10,15),
  beta2=c(0.2,0.4),
...
bake(file = "output/round_01/ovov_mif_01.rds",{ 
  mifs_global_01 <- foreach(i=1:ncores,.packages='pomp', .options.multicore=mcopts) %dopar% {
    set.seed(112*i)
    panelPomp::mif2(
      sir_panel,
      Np = NP_MIF,
...
bake(file = "output/round_01/ovov_01_el.rds",{
  set.seed(33124)
  el_01 <- measlespkg::eval_logLik(mifs_global_01, ncores=ncores, np_pf = NP_MIF, nreps=ncores)
  el_01
})
@
...
<<maximization2, include=False, massage=False, warning=False>>=
set.seed(1234)
sd_02           = 2/3

el_01 <- readRDS("output/round_01/ovov_01_el.rds")
...
bake(file = "output/round_02/ovov_mif_02.rds",{ 
  mifs_global_02 <- foreach(i=1:ncores,.packages='pomp', .options.multicore=mcopts) %dopar% {
    set.seed(202407222+1234*i)
    panelPomp::mif2(
      sir_panel,
      Np = NP_MIF,
      cooling.fraction.50 = 0.5,
      rw.sd = rw_sd(beta1=0.01*sd_02,beta2=0.01*sd_02,beta3=0.01*sd_02,
                    beta11=0.01*sd_02,phi=0.01*sd_02,sigma_q=0.01*sd_02,sigma_xi=0.01*sd_02,
      ...
      Nmif = NMIF,
      shared.start = numeric(0),
      specific.start = starting_values_02[[i]],
      block = F
    ) 
  }
  mifs_global_02
...
bake(file = "output/round_02/ovov_02_el.rds",{
  set.seed(4321)
  el_02 <- measlespkg::eval_logLik(mifs_global_02, ncores = ncores, np_pf = NP_MIF, nreps=ncores)
  el_02
})
@
...
<<maximization3, include=False, massage=False, warning=False>>=
set.seed(12345)
sd_03           = (2/3)^2

el_02 <- readRDS("output/round_02/ovov_02_el.rds")
...
bake(file = "output/round_03/ovov_mif_03.rds",{ 
  mifs_global_03 <- foreach(i=1:ncores,.packages='pomp', .options.multicore=mcopts) %dopar% {
    set.seed(2207+4321*i)
    panelPomp::mif2(
      sir_panel,
      Np = NP_MIF,
      cooling.fraction.50 = 0.5,
      rw.sd = rw_sd(beta1=0.01*sd_03,beta2=0.01*sd_03,beta3=0.01*sd_03,
                    beta11=0.01*sd_03,phi=0.01*sd_03,sigma_q=0.01*sd_03,sigma_xi=0.01*sd_03,
      ...
      Nmif = NMIF,
      shared.start = numeric(0),
      specific.start = starting_values_03[[i]],
      block = F
    ) 
  }
  mifs_global_03
...
bake(file = "output/round_03/ovov_03_el.rds",{
  set.seed(1234)
  el_03 <- measlespkg::eval_logLik(mifs_global_03, ncores = ncores, np_pf = NP_MIF, nreps=ncores)
  el_03
})
@


<<estimated-mle, include=False, massage=False, warning=False>>=
mif_el <- readRDS("output/round_03/ovov_03_el.rds") # Can be changed if more rounds needed
est_mle_vec <- mif_el$fits[which.max(mif_el$fits$logLik),]
@
...
\begin{table}[htbp]
  ...
  \begin{threeparttable}
    ...
    \begin{tabular}{>{\raggedright\arraybackslash}p{2cm} *{3}{>{\centering\arraybackslash}p{1.5cm}} >{\centering\arraybackslash}p{2.5cm}}
      ...
      \midrule
      \(\beta_1\)    & 12.15 & 12.74 & 11.48 & $\Sexpr{est_mle_vec["beta1[unit1]"]}$\\
      \(\beta_2\)    & 0.22  & 0.21  & 0.25  & $\Sexpr{est_mle_vec["beta2[unit1]"]}$\\
      \(\beta_3\)    & 0.34  & 0.31  & 0.35  & $\Sexpr{est_mle_vec["beta3[unit1]"]}$\\
      \(\phi\)       & 0.017 & 0.14  & 0.14  & $\Sexpr{est_mle_vec["phi[unit1]"]}$\\
      \(\rho\)       & 0.022 & 0.19  & 0.16  & $\Sexpr{est_mle_vec["rho[unit1]"]}$\\
      \(\sigma^2_q\) & n/a   & 0.042 & 0.021 & $\Sexpr{est_mle_vec["sigma_q[unit1]"]}$\\
      \(\sigma_\xi\) & n/a   & n/a   & 66.89 & $\Sexpr{est_mle_vec["sigma_xi[unit1]"]}$\\
      \addlinespace
      \multicolumn{1}{l}{\(S_{10}\)} & \multicolumn{3}{c}{$3876549^*$} & $$\Sexpr{est_mle_vec["S10[unit1]"]}$$\\
      \multicolumn{1}{l}{\(I_{10}\)} & \multicolumn{3}{c}{$30351$}     & $$\Sexpr{est_mle_vec["I10[unit1]"]}$$\\
      \multicolumn{1}{l}{\(R_{10}\)} & \multicolumn{3}{c}{$1315221$}   & $$\Sexpr{est_mle_vec["R10[unit1]"]}$$\\
      \multicolumn{1}{l}{\(S_{20}\)} & \multicolumn{3}{c}{$57139612$}  & $$\Sexpr{est_mle_vec["S20[unit1]"]}$$\\
      \multicolumn{1}{l}{\(I_{20}\)} & \multicolumn{3}{c}{$871$}       & $$\Sexpr{est_mle_vec["I20[unit1]"]}$$\\
      \multicolumn{1}{l}{\(R_{20}\)} & \multicolumn{3}{c}{$302852$}    & $$\Sexpr{est_mle_vec["R20[unit1]"]}$$\\
      \multicolumn{1}{l}{\(S_{30}\)} & \multicolumn{3}{c}{$19573727$}  & $$\Sexpr{est_mle_vec["S30[unit1]"]}$$\\
      \multicolumn{1}{l}{\(I_{30}\)} & \multicolumn{3}{c}{$2550$}      & $$\Sexpr{est_mle_vec["I30[unit1]"]}$$\\
      \multicolumn{1}{l}{\(R_{30}\)} & \multicolumn{3}{c}{$131092$}    & $$\Sexpr{est_mle_vec["R30[unit1]"]}$$\\
      \midrule
      \addlinespace
      \textbf{AIC}  & $98866.65$ & $15154.75$ & $13778.08$ & $\Sexpr{(-2)*((length(wwr_mle) - length(params_fixed)) - est_mle_vec["logLik"])}$\\
      \addlinespace
      \bottomrule
    \end{tabular}
    \begin{tablenotes}
