...
We show that the comparison of log-likelihood values made by \cite{wwr} is flawed because the PAL calculations were carried out using a dataset scaled differently from the previous study.
...
<<maximization7-Stocks_rinit, echo=F, warning=F, message=F>>=
set.seed(12345)
sd_07 = (2/3)^6
starting_values_sbh_07 <- choose_n_fits(sbh_el_06, top_n_fits, ncores)

bake(file = paste0(out_dir, "mif2_maximization/SBH_init/round_07/mifs_global_sbh_07.rds"),{ 
  plan(multicore, workers = ncores)
  foreach(i = 1:ncores, .options.future = list(seed = 652643293)) %dofuture% {
    pomp::mif2(
      mifs_global_sbh_06[[1]],
      rw.sd = rw_sd(
        beta1 = 0.01*sd_07, beta2 = 0.01*sd_07, beta3 = 0.01*sd_07,
        beta11 = 0.01*sd_07, phi = 0.01*sd_07,
        sigma_q = 0.01*sd_07, sigma_xi = 0.01*sd_07
      ),
      params = starting_values_sbh_07[[i]]
    ) 
  }
}) -> mifs_global_sbh_07


bake(file = paste0(out_dir,"mif2_maximization/SBH_init/round_07/sbh_el_07.rds"),{
  eval_logLik_pomp(mifs_global_sbh_07, ncores, NP_MIF, seed = 4248930)
}) ->  sbh_el_07
@

<<estimated-mle_stocks_init, echo=FALSE, message=FALSE, warning=FALSE>>=
# Can be changed if more rounds needed
est_mle_vec_sbh <- sbh_el_07[which.max(sbh_el_07$logLik),]
@
...
This theory is supported by numerical results, in their Table~1, which erroneously claim to show that PAL has substantially stronger performance than a particle filter (PF) on an example of scientific interest but moderate size.
...
Table~1 of WWR reanalyzes the model and data of \citet{stocks} (henceforth, SBH) for which the likelihood was calculated using PF.
SBH found clear evidence for the importance of overdispersion in a stochastic dynamic model for their epidemiological data.
...
We show that most of the apparent advantage for PAL over PF, on the OvOv model, arises because WWR used a different scaling of the data from SBH.
...
arma_benchmark <- function(x,order=c(2,0,1)){
  arima(log(x + 1), order = order)$loglik - sum(log(x+1))
}

sum(apply(realdat,2, function(x)  arima(log(x + 1), order =c(2,0,1))$aic + 2 * sum(log(x+1))))

sbh_data <- read.csv("sbh_data.txt",sep=" ",skip=3)[,-1]
...
\centering 
\caption{AIC for the OvOv rotavirus model, computed using two filtering methods. PAL is the Poisson approximate likelihood, implemented using the code of WWR. PF is the particle filter, implemented using the pomp R package \citep{pomppackagepaper}. Lines 1, 2 and 7 are from WWR, and the remainder are our own computations. We used $5\times 10^4$ particle for both PF and PAL. PF was repeated 36 times to reduce the Monte Carlo variance, but this step was not necessary for PAL due to its lower Monte~Carlo variance. PF was maximized using iterated filtering, following the approach of SBH. PAL was maximized using coordinate gradient descent, using the code of WWR.
} 
...
\begin{tabular}{lllr} 
  ...
  & Method & Result & AIC  \\
  \hline
  1. & PF & Table~1 of WWR, originally from SBH &  \Sexpr{myround(20134.38,tab_signif)}
  \\
  2. & PAL & Table~1 of WWR & \Sexpr{myround(13778.08,tab_signif)}
  \\
  3. & PAL & Data, model and parameters from WWR &
    \Sexpr{myround(-2*palfilter_lik_list_wwr$log_lik + 2*7 ,tab_signif)} 
  \\
  4. & PF & Data, model and parameters matching 3. &
    \Sexpr{myround(-2*logmeanexp(logLik(pfilter_wwr)+ 2*7),tab_signif)}
  \\
  5. & PF & Changing initial values &
     \Sexpr{myround(2*((length(params_wwr) - length(params_fixed)) - est_mle_vec["logLik"]),tab_signif)}
  \\
  6. & PAL & Data, model and parameters matching 5.  &
     \Sexpr{myround(-2*palfilter_mif2_mle$log_lik + 2*7 ,tab_signif)}
  \\   
  7. & ARMA & Sec.~7.3.2 of WWR & 23043
  \\
  8. & ARMA & Data from WWR &
...
On $3\times 416$ data points, this corresponds to a discrepancy of $-1248 \, \log(0.07) \approx 3300$ log-likelihood units, largely explaining the difference interpreted by WWR as evidence supporting PAL.
...
Inspection of log-likelihood anomalies \citep{wheeler24,li24} showed that the initial conditions for the latent process in January 2001 were fixed at values which were incompatible with the trajectory of the data early in the time series.
By contrast, SBH fixed their initial conditions 6 years before the first measurement, giving time for the system to reach its equilibrum distribution.
Line 5 of Table~\ref{tab:ovovrealdata} incorporates this into the model of WWR and shows that this improvement enables PF to reach the values attained by PAL.
Further, at the MLE obtained by maximizing PF, we found that PF beats PAL (comparing lines 5 and 6).

WWR compared their fitted models with a log-ARMA(2,1) benchmark AIC value of 23043 (line 7).
From this high AIC value, they inferred that all the mechanistic models possessing overdispersion have better statistical fit than a simple loglinear time series model.
However, this AIC value corresponds to SBH's data, not the data fitted by WWR.
Thus, line 7 can properly compared to line 1 but not to any other line of  Table~\ref{tab:ovovrealdata}. 
We independently fitted a log-ARMA(2,1) model to the SBH data and obtained a similar AIC value (\Sexpr{myround(sbh_benchmark,tab_signif)}).
...
We know from the benchmark AIC value in Table~\ref{tab:ovovrealdata}. line 8, that there is substantial model misspecification.

It is currently unclear why the model of SBH beats the log-ARMA benchmark for the data they fitted to, whereas the model of WWR fits more poorly than the log-ARMA model for its corresponding data.
...
Particle filter methods meet this criterion since they have the plug-and-play property \citep{breto09,he10} which is not posessed by PAL.
...
Also contained dditional results reinforcing the conclusions presented in this article.
