...
<<OvOv-model, echo=FALSE, message=FALSE, warning=FALSE, results='hide'>>=
# measurement model 
dmeas <- Csnippet("
                  if (ISNA(cases1)) {
                  lik = (give_log) ? 0 : 1;
...
<<maximization1, echo=FALSE, message=FALSE, warning=FALSE, results='hide'>>=
set.seed(123)
sir_box <- list(
  beta1=c(10,15),
  beta2=c(0.2,0.4),
...
<<maximization2, echo=FALSE, message=FALSE, warning=FALSE, results='hide'>>=
set.seed(1234)
sd_02 = 2/3
...
<<maximization3, echo=FALSE, message=FALSE, warning=FALSE, results='hide'>>=
set.seed(12345)
sd_03           = (2/3)^2
...
<<pf-sim-100, echo=FALSE, results='hide', message=FALSE, warning=FALSE>>=
# Set WWR's MLE
wwr_mle_raw <- c(params_fixed, 
             "beta1" = 11.48,
             "beta2" = 0.25,
...
<<plot1, echo=FALSE, fig.height=4, fig.width=5.5>>=
# Create your data
time <- 1:416
...
ggplot(df, aes(x = time, y = cond_loglik, group = interaction(method, factor(1:ncores)), color = method)) +
  geom_line(data = filter(df, method == "SMC"), color = "blue") +
  geom_line(data = filter(df, method == "PAL"), color = "red") +
  theme_minimal() +
