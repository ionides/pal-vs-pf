...
\begin{tabular}{llllr} 
  \hline
  & Method & Data & Model & AIC  \\
  \hline
  1. & PF & Rescaled counts & SBH &  \Sexpr{myround(20134.38,tab_signif)}
  \\
  2. & PAL & Counts &  WWR & \Sexpr{myround(13778.08,tab_signif)}
  \\
  3. & PAL & Counts & Re-evaluating WWR at published parameters &
    \Sexpr{myround(-2*palfilter_lik_list_wwr$log_lik + 2*7 ,tab_signif)} 
  \\
  4. & PF & Counts & Model and parameters matching line 3 &
    \Sexpr{myround(-2*logmeanexp(logLik(pfilter_wwr)) + 2*7,tab_signif)}
  \\
  5. & PF & Counts & WWR with initial values changed to SBH &
     \Sexpr{myround(-2*est_mle_vec_sbh["logLik"] + 2*7,tab_signif)}
  \\
  6. & PAL & Counts & Model and parameters matching line 5  &
     \Sexpr{myround(-2*palfilter_mif2_mle$log_lik + 2*7 ,tab_signif)}
  \\   
  7. & Benchmark & Rescaled counts  & log-ARMA$(2,1)$ & 23043
  \\
  8. & Benchmark & Counts           &  log-ARMA$(2,1)$ & \Sexpr{myround(wwr_benchmark,tab_signif)} 
  \\
 \hline
\end{tabular}
\end{table}
